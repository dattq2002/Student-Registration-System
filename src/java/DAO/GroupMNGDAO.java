        ...
        try {
            ...
            if (conn != null) {
                String sql = "select s.Code as SubjectCode, s.ID as SubjectID, na.CourseName, na.CourseCode, na.CourseID "
                        ...
                        + "where c.SemesterID = 11114 and sic.LecturerID = (select ID from Lecturer where Email = ?) and sic.Status = 1) "
        ...
        try {
            ...
            if (conn != null) {
                String sql = "select na.*, s.Code as SubjectCode "
                        ...
                        + "ON na.SubjectID = s.ID "
                        + "where na.LecturerID = (select ID from Lecturer where Email = ?) and na.Status = 'True' and na.CourseCode = ? and s.ID = ?";
                stm = conn.prepareStatement(sql);
                stm.setString(1, email);
                stm.setInt(2, cID);
                stm.setInt(3, sID);
